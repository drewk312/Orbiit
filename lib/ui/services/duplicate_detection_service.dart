...
class DuplicateDetectionService {
  ...
  Future<Map<String, List<DuplicateEntry>>> findDuplicates(
    ...
    for (int i = 0; i < games.length; i++) {
      ...
      final baseId = game.gameId.length >= 4 ? game.gameId.substring(0, 4) : game.gameId;
    ...
    if (checkCRC && duplicates.isNotEmpty) {
      AppLogger.info('Calculating CRC32 for ${duplicates.length} duplicate groups...', 'DuplicateDetection');
      
      int processedGroups = 0;
      for (final group in duplicates.values) {
        for (final entry in group) {
          try {
            entry.crc32 = await _checksumService.calculateCRC32File(entry.filePath);
          } catch (e) {
            AppLogger.error('Error calculating CRC for ${entry.filePath}', 'DuplicateDetection', e);
          }
        }
        processedGroups++;
  ...
  Future<List<List<DuplicateEntry>>> findExactDuplicates(
    ...
    for (int i = 0; i < games.length; i++) {
      ...
      try {
        ...
        AppLogger.error('Error processing ${game.filePath}', 'DuplicateDetection', e);
      }
    }
