...
class MemoryCardManagerScreen extends StatefulWidget {
  ...
  @override
  State<MemoryCardManagerScreen> createState() => _MemoryCardManagerScreenState();
}
...
class _MemoryCardManagerScreenState extends State<MemoryCardManagerScreen> {
  ...
  Future<void> _pickLibrary() async {
    final result = await FilePicker.platform.getDirectoryPath(dialogTitle: 'Select Game Library Root');
    if (result != null) {
      ...
      body: Column(
        children: [
            ImmersiveGlassHeader(
              ...
              leading: Icon(Icons.sd_storage_rounded, color: FusionColors.nebulaViolet),
              actions: [
                IconButton(icon: const Icon(Icons.folder_open), onPressed: _pickLibrary, tooltip: 'Identify Library Root'),
                IconButton(icon: const Icon(Icons.add), onPressed: _libraryRoot == null ? null : _showCreateDialog, tooltip: 'New Card'),
              ],
            ),
            ...
            if (_libraryRoot == null)
              Expanded(
                child: Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const Text('Select your Game Library folder to scan for saves.', style: TextStyle(color: Colors.white54)),
                      const SizedBox(height: 16),
                      ElevatedButton(onPressed: _pickLibrary, child: const Text('Select Folder')),
                    ],
                  ),
                ),
              )
               ...
               const Expanded(child: Center(child: Text('No memory cards found in /saves.', style: TextStyle(color: Colors.white54))))
            else
               Expanded(
                 child: ListView.builder(
                   ...
                   itemBuilder: (context, index) {
                     ...
                     final sizeMb = (card.lengthSync() / (1024*1024)).toStringAsFixed(1);
                     return Card(
                       color: FusionColors.surfaceCard,
                       margin: const EdgeInsets.only(bottom: 12),
                       child: ListTile(
                         leading: const Icon(Icons.save, color: FusionColors.nebulaViolet),
                         title: Text(path.basename(card.path), style: const TextStyle(color: Colors.white)),
                         subtitle: Text('$sizeMb MB', style: const TextStyle(color: Colors.white54)),
                         trailing: Row(
                           mainAxisSize: MainAxisSize.min,
                           children: [
                             IconButton(
                               icon: const Icon(Icons.backup, color: FusionColors.nebulaCyan),
  ...
  void _showCreateDialog() {
    showDialog(context: context, builder: (_) => _CreateCardDialog(
      onCreate: (name, size) async {
        try {
...
class _CreateCardDialogState extends State<_CreateCardDialog> {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      title: const Text('Create Memory Card', style: TextStyle(color: Colors.white)),
      content: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          TextField(
            ...
            decoration: const InputDecoration(labelText: 'Filename', labelStyle: TextStyle(color: Colors.white70)),
          ),
          const SizedBox(height: 16),
          DropdownButton<int>(
            value: _selectedSize,
            ...
            isExpanded: true,
            items: const [
              DropdownMenuItem(value: MemoryCardService.size59, child: Text('59 Blocks (512KB)')),
              DropdownMenuItem(value: MemoryCardService.size123, child: Text('123 Blocks (1MB)')),
              DropdownMenuItem(value: MemoryCardService.size251, child: Text('251 Blocks (2MB)')),
              DropdownMenuItem(value: MemoryCardService.size507, child: Text('507 Blocks (4MB)')),
              DropdownMenuItem(value: MemoryCardService.size1019, child: Text('1019 Blocks (8MB)')),
              DropdownMenuItem(value: MemoryCardService.size2043, child: Text('2043 Blocks (16MB)')),
            ],
