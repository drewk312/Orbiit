...
class _SpaceBackgroundState extends State<SpaceBackground>
  ...
  void initState() {
    ...
    // Generate stars once
    _stars = _generateStars(100);
    _nebulaClouds = _generateNebulaClouds(3);
    ...
    return List.generate(count, (i) {
      ...
      return _Star(
        ...
        size: type < 0.6 
            ...
            : type < 0.9
                ? 1.0 + random.nextDouble() * 0.5 // 30% normal stars
                : 1.5 + random.nextDouble() * 1.0, // 10% bright stars
        opacity: type < 0.6
            ? 0.15 + random.nextDouble() * 0.15  // dim dust
            : type < 0.9
                ? 0.3 + random.nextDouble() * 0.3  // normal
                : 0.6 + random.nextDouble() * 0.3, // bright
  ...
  Widget build(BuildContext context) {
    return Stack(
      fit: StackFit.expand,
      children: [
        // Layer 1: Deep space gradient (static)
        Container(
          decoration: BoxDecoration(
            gradient: RadialGradient(
              ...
              radius: 1.2,
              colors: [
                const Color(0xFF12101A), // Slightly purple center
                OrbColors.bgPrimary,
                OrbColors.void_,
              ],
  ...
  void paint(Canvas canvas, Size size) {
    for (final star in stars) {
      ...
      if (star.twinkle) {
        ...
        final twinkleFactor = 0.5 + 0.5 * math.sin(twinkleValue * 2 * math.pi + star.x * 10);
        opacity = star.opacity * (0.5 + 0.5 * twinkleFactor);
      }
