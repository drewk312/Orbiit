...
abstract final class _Config {
  ...
  static const pollIntervalMs = 100;
...
class ForgeProvider extends ChangeNotifier {
  ...
  ForgeProvider({String? persistenceDir})
  ...
  Future<void> _initializeNativeLibrary() async {
    try {
      ...
      if (!success) {
        debugPrint('[ForgeProvider] WARNING: Native init failed - retrying with temp dir');
  ...
  Future<bool> initialize() async {
    try {
      ...
      if (success) {
        ...
        _error = 'Failed to initialize forge backend';
        debugPrint('[ForgeProvider] Failed to initialize forge backend');
      }
  ...
  Future<void> _startForgeInternal(GameResult game,
    ...
    try {
      ...
      // PATCH: Fix legacy Myrient URLs sticking in the queue with .rvz extension
      // (Myrient lists .rvz files wrapped in .zip)
      if (targetUrl.contains('myrient.erista.me') && targetUrl.endsWith('.rvz')) {
        debugPrint('[ForgeProvider] Patching legacy .rvz URL to .zip');
        targetUrl = targetUrl.replaceAll('.rvz', '.zip');
      }
      ...
      if (!_forgeBridge.isNativeReady && !_forgeBridge.isMockMode) {
        ...
        // Wait for native library to initialize (max 5 seconds)
        int waitMs = 0;
        while (!_forgeBridge.isNativeReady && waitMs < 5000) {
        ...
        if (!_forgeBridge.isNativeReady) {
          throw Exception(
              'Native library failed to initialize after 5 seconds');
        }
      }
  ...
  /// Pause current forge operation
  Future<void> pauseForge() async {
    if ((!_isForging && _currentStatus != ForgeStatus.paused) || _currentMissionId == null) return;
    
    debugPrint('[ForgeProvider] Pausing mission: $_currentMissionId');
    final success = _forgeBridge.pauseMission(_currentMissionId!);
  ...
  /// Resume paused forge operation
  Future<void> resumeForge() async {
    if (_currentMissionId == null || _currentStatus != ForgeStatus.paused) return;

    debugPrint('[ForgeProvider] Resuming mission: $_currentMissionId');
    ...
    final success = _forgeBridge.resumeMission(_currentMissionId!, progressCallback);
  ...
  Future<void> cancelForge() async {
    ...
    // Store current mission ID before any state changes
  ...
  void handleProgressUpdate(int status, double progress, String message) {
    ...
    if (_lastProgressTimestamp != null) {
      ...
      // Clamp progress and ignore slight regressions (can happen due to native rounding)
      final clampedProgress = progress.clamp(0.0, 1.0);
      var dp = clampedProgress - _lastProgressValue;
      if (dp < 0) dp = 0; // ignore negative regressions
      if (dt > 0) {
        _progressSpeed = dp / dt; // percent per second (0-1 scale)
      }

      // Bytes/sec: only update on positive deltas, apply light smoothing
      if (downloadedBytes != null && _lastDownloadedBytes != null) {
        ...
        if (deltaBytes > 0 && dt > 0) {
          final newBps = deltaBytes / dt;
          // Smooth to avoid jitter (alpha blend)
          const alpha = 0.35;
          if (_downloadSpeedBps == null || _downloadSpeedBps == 0) {
