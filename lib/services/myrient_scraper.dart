  ...
  Future<String?> findGameUrl(String baseUrl, String query) async {
    ...
    try {
      ...
      if (response.statusCode != 200) {
        debugPrint("[MyrientScraper] Failed to reach Myrient: ${response.statusCode}");
        return null;
      }

      final htmlBody = await response.transform(SystemEncoding().decoder).join();
      ...
      for (var anchor in anchors) {
        ...
        try { text = Uri.decodeComponent(text); } catch (_) {}
        
        final lower = text.toLowerCase();
        
        // Check extensions (zip for No-Intro, rvz for Redump, etc)
        if (!lower.endsWith('.zip') && !lower.endsWith('.rvz') && 
