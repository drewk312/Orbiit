...
class GameMetadataService {
  ...
  Future<GameMetadata> getGameMetadata(String title, String platform) async {
    ...
    debugPrint('[GameMetadata] Fetching metadata for: $title ($platform)');
    ...
    final metadata = GameMetadata(
      ...
      description: wikiData?['description'] ?? _generateDescription(title, platform),
      coverUrl: coverUrl,
  ...
  Future<Map<String, dynamic>?> _fetchWikipediaData(String title, {String? platform}) async {
    try {
      ...
      if (titleLower.contains('wii u') || titleLower.contains('wiiu')) {
      ...
      } else if (titleLower.contains('3ds') || titleLower.contains('3d land') || titleLower.contains('3d world')) {
        ...
        if (platformLower.contains('n64') || platformLower.contains('nintendo 64')) {
          searchQuery = '$searchTitle Nintendo 64 video game';
        } else if (platformLower.contains('snes') || platformLower.contains('super nintendo')) {
          searchQuery = '$searchTitle Super Nintendo video game';
        } else if (platformLower.contains('nes') && !platformLower.contains('snes')) {
          searchQuery = '$searchTitle NES video game';
        } else if (platformLower.contains('gba') || platformLower.contains('game boy advance')) {
          searchQuery = '$searchTitle Game Boy Advance';
        } else if (platformLower.contains('gbc') || platformLower.contains('game boy color')) {
          searchQuery = '$searchTitle Game Boy Color';
        } else if (platformLower.contains('wii') && !platformLower.contains('wiiu') && !platformLower.contains('wii u')) {
          // Platform is Wii but title doesn't have "Wii" - add it explicitly
          searchQuery = '$searchTitle Wii video game';
        } else if (platformLower.contains('wiiu') || platformLower.contains('wii u')) {
          searchQuery = '$searchTitle Wii U video game';
        } else if (platformLower.contains('gamecube') || platformLower.contains('gc')) {
          searchQuery = '$searchTitle GameCube video game';
        } else if (platformLower.contains('genesis') || platformLower.contains('mega drive')) {
      ...
      debugPrint('[GameMetadata] Wikipedia search: $searchQuery');
      ...
      debugPrint('[GameMetadata] Wikipedia data found for: $title');
      return parsed;
      
    } catch (e) {
      debugPrint('[GameMetadata] Wikipedia fetch error: $e');
      return null;
    }
  }
  ...
  Map<String, dynamic> _parseWikipediaExtract(String extract, String title) {
    ...
    final devMatch = RegExp(r'developed by ([^.]+)', caseSensitive: false).firstMatch(extract);
    if (devMatch != null) {
      result['developer'] = devMatch.group(1)?.trim();
    }

    // Try to extract publisher
    final pubMatch = RegExp(r'published by ([^.]+)', caseSensitive: false).firstMatch(extract);
    if (pubMatch != null) {
      result['publisher'] = pubMatch.group(1)?.trim();
    }

    // Try to extract release year
    final yearMatch = RegExp(r'released.*?(\d{4})|(\d{4}).*?release', caseSensitive: false).firstMatch(extract);
    if (yearMatch != null) {
    ...
    final genrePatterns = [
      'role-playing', 'RPG', 'action', 'adventure', 'platformer', 'puzzle',
      'racing', 'sports', 'fighting', 'shooter', 'simulation', 'strategy',
      'horror', 'survival', 'stealth', 'rhythm', 'party', 'educational'
    ];
  ...
  Future<String?> _fetchCoverArt(String title, String platform) async {
    try {
      ...
      if (cleanTitle.toLowerCase().contains('zelda')) {
        ...
        final zeldaMatch = RegExp(r'The Legend of Zelda[:\s]*(.*)').firstMatch(cleanTitle);
        if (zeldaMatch != null) {
      ...
      for (final name in candidates) {
        ...
        final url = 'https://raw.githubusercontent.com/libretro-thumbnails/$system/master/Named_Boxarts/$encoded.png';
        ...
        try {
          ...
          if (res.statusCode == 200) {
            debugPrint('[GameMetadata] Found cover art: $url');
            return url;
          }
        } catch (_) {
          continue;
      ...
      debugPrint('[GameMetadata] Cover art fetch error: $e');
      return null;
    }
  }
  ...
  String _cleanTitleForSearch(String title) {
    ...
    // Remove Rev info
    clean = clean.replaceAll(RegExp(r'\s*Rev\s*\d+.*', caseSensitive: false), '');
    // Normalize spaces
  ...
  String _getLibretroSystem(String platform) {
    ...
    if (p == 'snes' || p == 'super nintendo') return 'Nintendo_-_Super_Nintendo_Entertainment_System';
    if ((p == 'nes' || p == 'nintendo') && !p.contains('super')) return 'Nintendo_-_Nintendo_Entertainment_System';
    if (p == 'gba' || p == 'game boy advance') return 'Nintendo_-_Game_Boy_Advance';
    if (p == 'gbc' || p == 'game boy color') return 'Nintendo_-_Game_Boy_Color';
    if (p == 'gb' || p == 'game boy') return 'Nintendo_-_Game_Boy';
    if (p == 'nds' || p == 'ds' || p == 'nintendo ds') return 'Nintendo_-_Nintendo_DS';
    if (p == '3ds' || p == 'nintendo 3ds') return 'Nintendo_-_Nintendo_3DS';
    if (p == 'genesis' || p == 'mega drive' || p == 'sega genesis') return 'Sega_-_Mega_Drive_-_Genesis';
    if (p == 'dreamcast' || p == 'sega dreamcast') return 'Sega_-_Dreamcast';
    if (p == 'saturn' || p == 'sega saturn') return 'Sega_-_Saturn';
    if (p == 'ps1' || p == 'psx' || p == 'playstation') return 'Sony_-_PlayStation';
    if (p == 'ps2' || p == 'playstation 2') return 'Sony_-_PlayStation_2';
    if (p == 'psp' || p == 'playstation portable') return 'Sony_-_PlayStation_Portable';
