...
class StorageOrganizerService {
  // Singleton
  static final StorageOrganizerService _instance = StorageOrganizerService._internal();
  ...
  Future<StorageAnalysis> analyzeDrive(String drivePath) async {
    ...
    for (final dupGroup in duplicates) {
      issues.add(StorageIssue(
        ...
        bytesAffected: dupGroup.skip(1).fold<int>(0, (sum, g) => sum + g.fileSize),
      ));
    }
  ...
  Future<StoredGame?> _analyzeGameFile(File file) async {
    try {
      ...
      final gameId = _extractGameId(fileName) ?? await _readGameIdFromFile(file);
  ...
  Future<(int total, int used, int free)> _getDriveSpace(String drivePath) async {
    try {
    ...
    // Default fallback
    return (500 * 1024 * 1024 * 1024, 250 * 1024 * 1024 * 1024, 250 * 1024 * 1024 * 1024);
  }
  ...
  Future<int> organizeGames(
    ...
    for (final game in analysis.games) {
      ...
      if (game.filePath != targetPath) {
        _progressController.add('${dryRun ? '[DRY RUN] Would move' : 'Moving'}: ${game.title}');
    ...
    _progressController.add('Organization complete: $movedCount games ${dryRun ? 'would be' : ''} moved');
    return movedCount;
  }
  
  /// Get target path for a game based on strategy
  String _getTargetPath(String drivePath, StoredGame game, OrganizationStrategy strategy) {
    ...
    switch (strategy) {
      ...
      case OrganizationStrategy.alphabetical:
        final firstChar = game.title.isNotEmpty ? game.title[0].toUpperCase() : '_';
  ...
  Future<int> removeDuplicates(
    ...
    for (final issue in duplicateIssues) {
      ...
      for (final duplicatePath in issue.relatedPaths!) {
        _progressController.add('${dryRun ? '[DRY RUN] Would remove' : 'Removing'}: $duplicatePath');
    ...
    _progressController.add('Duplicate removal complete: $removedCount files ${dryRun ? 'would be' : ''} removed');
    return removedCount;
  }
