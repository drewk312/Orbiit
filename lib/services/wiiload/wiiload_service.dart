...
class WiiLoadService {
  ...
  Future<void> startDiscovery({Duration interval = const Duration(seconds: 10)}) async {
    stopDiscovery();
    _isDiscovering = true;
  ...
  Future<bool> testConnection(String wiiIp) async {
    try {
      debugPrint('[WiiLoad] Testing connection to $wiiIp:${WiiLoadProtocol.port}');
  ...
  Future<bool> sendExecutable(
    ...
    try {
      ...
      final header = _buildWiiLoadHeader(
        fileBytes.length,
        fileType == WiiFileType.dol ? WiiLoadProtocol.typeDol : WiiLoadProtocol.typeElf,
        arguments,
      );
  ...
  Uint8List _buildWiiLoadHeader(int fileSize, int fileType, String? arguments) {
    final argBytes = arguments != null 
        ? utf8.encode(arguments) 
        : Uint8List(0);
