...
class EnhancedDownloadService {
  static final EnhancedDownloadService _instance = EnhancedDownloadService._internal();
  ...
  Future<EnhancedDownloadTask> addDownload({
    ...
    if (gameId != null || title.isNotEmpty) {
      try {
        ...
        for (final result in searchResults) {
          if (result.downloadUrl != null && !sourceUrls.contains(result.downloadUrl)) {
    ...
    if (sourceUrls.isEmpty) {
      debugPrint('[EnhancedDownload] No sources found, using fallback discovery');
    }
  ...
  Future<void> _downloadWithFallback(EnhancedDownloadTask task) async {
    ...
    for (int i = task.currentSourceIndex; i < task.sourceUrls.length; i++) {
      ...
      try {
        debugPrint('[EnhancedDownload] Trying source ${i + 1}/${task.sourceUrls.length}: $url');
        ...
        if (task.isVerified || (task.expectedSHA1 == null && task.expectedMD5 == null)) {
        ...
        debugPrint('[EnhancedDownload] Hash verification failed, trying next source');
        task.retryCount++;
  ...
  Future<void> _executeDownload(EnhancedDownloadTask task, String url) async {
    ...
    try {
      ...
      if (task.totalBytes > 0 && actualSize != task.totalBytes) {
        throw Exception('File size mismatch: expected ${task.totalBytes}, got $actualSize');
      }
  ...
  Future<void> _verifyHash(EnhancedDownloadTask task) async {
    ...
    try {
      ...
      if (task.expectedSHA1 != null) {
        task.calculatedSHA1 = await _checksumService.calculateSHA1File(task.destinationPath!);
        if (task.calculatedSHA1!.toUpperCase() != task.expectedSHA1!.toUpperCase()) {
          throw Exception('SHA-1 mismatch: expected ${task.expectedSHA1}, got ${task.calculatedSHA1}');
        }
      }
      
      if (task.expectedMD5 != null) {
        task.calculatedMD5 = await _checksumService.calculateMD5File(task.destinationPath!);
        if (task.calculatedMD5!.toUpperCase() != task.expectedMD5!.toUpperCase()) {
          throw Exception('MD5 mismatch: expected ${task.expectedMD5}, got ${task.calculatedMD5}');
        }
      }
