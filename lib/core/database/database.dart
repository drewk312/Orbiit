...
class AppDatabase extends _$AppDatabase {
  ...
  Future<int> deleteTitle(int id) => (delete(titles)..where((t) => t.id.equals(id))).go();
  Future<int> deleteTitleByPath(String filePath) async {
    // First delete associated issues
    final title = await (select(titles)..where((t) => t.filePath.equals(filePath))).getSingleOrNull();
    if (title != null) {
  ...
  Future<List<Issue>> getDuplicateIssues() =>
      (select(issues)
        ..where((i) => i.issueType.equals('duplicate') & i.resolvedTimestamp.isNull())
  ...
  Future<PatchedRom?> getPatchedRomById(String id) async {
    final results = await (select(patchedRoms)..where((p) => p.id.equals(id))).get();
    return results.isEmpty ? null : results.first;
  }
