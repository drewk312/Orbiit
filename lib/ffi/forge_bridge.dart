...
class _DartMission {
  ...
  _DartMission(this.id, this.url, this.destPath) 
      : client = HttpClient(),
...
class ForgeBridge {
  ...
  void _initializeNativeLibrary() {
    ...
    try {
      ...
      _forgeInit = _lib!.lookupFunction<
          ffi.Int32 Function(),
          int Function()>('forge_init');
      ...
      _forgeScanFolder = _lib!.lookupFunction<
          ffi.Int32 Function(ffi.Pointer<Utf8>, ffi.Bool,
              ffi.Pointer<ffi.NativeFunction<ForgeGameFoundCallbackNative>>),
          int Function(ffi.Pointer<Utf8>, bool,
              ffi.Pointer<ffi.NativeFunction<ForgeGameFoundCallbackNative>>)>(
          'forge_scan_folder');
      ...
      _forgeCancelMission =
          _lib!.lookupFunction<ffi.Bool Function(ffi.Uint64),
              bool Function(int)>('forge_cancel_mission');
      ...
      try {
        _forgeDeployStructure =
            _lib!.lookupFunction<ffi.Bool Function(ffi.Pointer<Utf8>),
      ...
      try {
        _forgeConvertIsoToWbfs = _lib!.lookupFunction<
            ffi.Uint64 Function(ffi.Pointer<Utf8>, ffi.Pointer<Utf8>,
                ffi.Pointer<ffi.NativeFunction<ForgeProgressCallbackNative>>),
            int Function(ffi.Pointer<Utf8>, ffi.Pointer<Utf8>,
                ffi.Pointer<ffi.NativeFunction<ForgeProgressCallbackNative>>)>(
        ...
        _forgeSplitWbfsFat32 = _lib!.lookupFunction<
        ...
        _forgeGetFileFormat =
            _lib!.lookupFunction<ffi.Pointer<Utf8> Function(ffi.Pointer<Utf8>),
                ffi.Pointer<Utf8> Function(ffi.Pointer<Utf8>)>(
                'forge_get_file_format');
      } catch (e) {
  ...
  /// Internal worker for Dart-based downloads
  Future<void> _runDartMission(_DartMission mission, ForgeProgressCallbackDart callback) async {
    final logger = AppLogger.instance;
    
    try {
      if (!mission.isMissionPaused) {
        mission.update(ForgeStatus.downloading, mission.progress, "Starting download...");
      }
      ...
      final uri = Uri.parse(mission.url);
      final request = await mission.client.getUrl(uri);
      
      // Resume logic: Add Range header if we have existing bytes
      if (existingBytes > 0) {
      ...
      // Handle partial content (206) or regular OK (200)
      if (response.statusCode != 200 && response.statusCode != 206) {
        // If range not satisfiable (e.g. file complete or server mismatch), verify
        if (response.statusCode == 416) {
           logger.warning('Range not satisfiable - possibly complete or wrong range');
           // Might be done?
        } else {
      ...
      if (mission.isCancelled || mission.isMissionPaused) return;
      
      // If 200 OK but we asked for Range, server ignored it. We must restart.
      if (existingBytes > 0 && response.statusCode == 200) {
        logger.warning('Server does not support resuming (sent 200 instead of 206). Restarting.');
        existingBytes = 0;
        // The file mode below is 'append', so we might need to truncate if we're restarting fully
        if (file.existsSync()) {
            file.deleteSync();
            file.createSync();
        }
      ...
      // Append if resuming, otherwise write
      final sink = file.openWrite(mode: FileMode.append);
      mission.fileSink = sink;
      
      final contentLength = response.contentLength + existingBytes; // Total size
      ...
      await for (var data in response) {
        if (mission.isCancelled || mission.isMissionPaused) {
           // Ensure we flush before stopping
           await sink.flush();
           break;
        }
        ...
        // Throttling: Only update progress every 100ms or 1% change
        final now = DateTime.now();
        if (contentLength > 0 && 
            (now.difference(lastProgressUpdate).inMilliseconds > 100 || 
             (downloaded - lastReportedBytes) > (contentLength * 0.01))) {
          
          final progress = downloaded / contentLength;
          final progressMsg = '${(downloaded / 1024 / 1024).toStringAsFixed(1)} MB / ${(contentLength / 1024 / 1024).toStringAsFixed(1)} MB';
          
          // Update internal state (for getMissionProgress)
          mission.update(ForgeStatus.downloading, progress, progressMsg);
          
          // Fire callback (for direct listeners)
          final ptr = progressMsg.toNativeUtf8();
          try {
            callback(ForgeStatus.downloading.value, progress.toDouble(), ptr);
          } finally {
            calloc.free(ptr);
          }
      ...
      await sink.flush();
      await sink.close();
      mission.fileSink = null;
      
      if (mission.isCancelled) {
        logger.info('Mission #${mission.id} cancelled by user');
      ...
      if (mission.isMissionPaused) {
        logger.info('Mission #${mission.id} paused by user');
        // We leave the file as is
        mission.update(ForgeStatus.paused, mission.progress, "Paused");
        return;
      }
      
      logger.info('Mission #${mission.id} completed successfully');
      
      // Final success state
      mission.update(ForgeStatus.ready, 1.0, "Complete");
      
      final finalMsgPtr = 'Complete'.toNativeUtf8();
      try {
        callback(ForgeStatus.ready.value, 1.0, finalMsgPtr);
      } finally {
        calloc.free(finalMsgPtr);
      }
      
    } catch (e) {
      if (mission.isCancelled) return; // Ignore errors if cancelled
      if (mission.isMissionPaused) return;
      
      logger.error('Mission #${mission.id} failed', error: e);
      mission.update(ForgeStatus.error, 0.0, "Error: $e");
      
      final errorPtr = 'Error: $e'.toNativeUtf8();
      try {
        callback(ForgeStatus.error.value, 0.0, errorPtr);
      } finally {
  ...
  bool cancelMission(int missionId) {
    ...
    if (_dartMissions.containsKey(missionId)) {
       ...
       if (!mission.isCancelled) {
         ...
         // Force close the client to abort download immediately
         try { mission.client.close(force: true); } catch (_) {}
  ...
  bool pauseMission(int missionId) {
    if (_dartMissions.containsKey(missionId)) {
      ...
      if (!mission.isCancelled && !mission.isMissionPaused) {
        ...
        // Force close client to stop network activity (will be resumed later via Range)
        try { mission.client.close(force: true); } catch (_) {}
        return true;
      }
    }
  ...
  bool resumeMission(int missionId, ForgeProgressCallbackDart callback) {
    if (_dartMissions.containsKey(missionId)) {
      final mission = _dartMissions[missionId]!;
      if (mission.isMissionPaused && !mission.isCancelled) {
  ...
  Future<bool> formatDrive(
      String drivePath, String label, ForgeProgressCallbackDart callback) async {
    final logger = AppLogger.instance;
    // Sanitize drive path (e.g., "E:\" -> "E")
    final driveLetter = drivePath.replaceAll(RegExp(r'[:\\]'), '').toUpperCase();
    ...
    logger.info('Starting Dart-based FAT32 format of drive $driveLetter: label=$label');
  ...
  bool verifyHash(String filePath, [String? expectedHash]) {
    ...
    try {
      ...
      AppLogger.instance.info('Hash verification: $match (Actual: $actualHash, Expected: $expected)');
      return match;
    } catch (e) {
  ...
  String getFileFormat(String filePath) {
    ...
    if (identity != null) {
      if (identity.formatId == 1) { // ISO or GCM
  ...
  GameIdentity? _scanFileHeader(File file) {
    try {
      ...
      if (header[0x18] == 0x5D && header[0x19] == 0x1C && 
          header[0x1A] == 0x9E && header[0x1B] == 0xA3) {
        isWii = true;
      }
      // GC Magic: 0xC2339F3D at 0x1C
      else if (header[0x1C] == 0xC2 && header[0x1D] == 0x33 && 
               header[0x1E] == 0x9F && header[0x1F] == 0x3D) {
        isGc = true;
      }
      // WBFS Magic: "WBFS" at 0x0
      else if (header[0] == 0x57 && header[1] == 0x42 && header[2] == 0x46 && header[3] == 0x53) {
        isWbfs = true;
      }
  ...
  int scanFolder(
    ...
    final logger = AppLogger.instance;
    logger.info('Starting Dart-based folder scan: $folderPath (recursive: $recursive)');
